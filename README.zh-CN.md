...
本 demo 只使用 `Channel=1` 的 VIO 实时位姿流。`Channel=1` 的 `type=26` 帧不包含来源标识，因此示例程序不会根据位姿帧推断 Stereo3/Stereo4 归属。算法状态只来自 `vio_frame_sys_state`；`pose fps` 只表示当前 `Channel=1` 位姿流正在被接收。
...
已使用 `net_vio_get_cfg` / `net_vio_set_cfg` 验证当前 SDK 动态库可调用文档接口，包括 `/System/version`、`/System/hardware_status`、`/Baton/config`、`/System/network` 和 `/Algorithm/disable/5`。
